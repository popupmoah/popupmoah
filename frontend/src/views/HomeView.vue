    ...
    <div class="max-w-7xl mx-auto">
      ...
      <div class="text-center mb-12">
        ...
        <p class="text-xl text-gray-600 mb-8">
          서울의 다양한 팝업스토어를 한눈에 확인하고 방문해보세요
        </p>
        <q-btn 
          color="primary" 
          size="lg" 
...
const categories = computed(() => [
  { name: '패션', icon: 'style', color: 'purple', count: stores.value.filter(s => s.category === '패션').length },
  { name: '식품', icon: 'restaurant', color: 'orange', count: stores.value.filter(s => s.category === '식품').length },
  { name: '뷰티', icon: 'face', color: 'pink', count: stores.value.filter(s => s.category === '뷰티').length },
  { name: '라이프스타일', icon: 'home', color: 'green', count: stores.value.filter(s => s.category === '라이프스타일').length }
])
